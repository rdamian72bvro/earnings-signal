...
public class DbBacktestService(EarningsSignalDbContext dbContext) : IBacktestService
{
    ...
    public async Task<IReadOnlyList<BacktestTradeDto>> GetBacktestTradesAsync(Guid backtestRunId, CancellationToken cancellationToken = default)
    {
        return await dbContext.BacktestTrades
            .AsNoTracking()
            .Where(trade => trade.BacktestRunId == backtestRunId)
            .OrderBy(trade => trade.EntryDate)
            .Select(trade => MapTrade(trade))
            .ToListAsync(cancellationToken);
    }
